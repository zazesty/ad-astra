...
- **Model (owner decision — latency over antifragility):** pin **`GEMINI_FLASH_LITE_VER`** in
  `modelPins.ts` (currently `3.5-flash-lite` → OR `google/gemini-3.5-flash-lite`, direct twin
  derived). Flash-lite is the fastest/cheapest tier; classifier wants min latency.
  - **No** `gemini-flash-lite-latest` on OR or AI Studio (re-checked 2026-08) — versioned pin only.
  - Failover: same gen OR → direct AI Studio on transient gateway fail → prefilter. **No** older-gen
    `models[]` cascade (dropped 2026-08; bump VER only next upgrade).
...
- No `gemini-flash-lite-latest` exists (OR or AI Studio) → classifier pins via
  `modelPins.GEMINI_FLASH_LITE_VER` (3.5-flash-lite); failover OR→direct same gen. (§2)
